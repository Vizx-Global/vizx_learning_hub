  ...
  useEffect(() => {
    if (isLive) {
      const interval = setInterval(() => {
        // Simulate real-time updates by highlighting random users
        ...
        <table className="w-full">
          <thead className="bg-muted/30 border-b border-border">
            <tr>
              ...
              {showAchievements && (
                <th className="text-left p-4 font-semibold text-foreground">Recent Achievements</th>
              )}
          ...
          <tbody>
            {data?.map((user, index) => (
              ...
              >
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    ...
                    {user?.pointsChange && (
                      ...
                      `}>
                        {user?.pointsChange > 0 ? '+' : ''}{user?.pointsChange}
                      </span>
                    )}
                  </div>
                </td>
                ...
                <td className="p-4">
                  <div className="flex items-center gap-2">
                    <Icon name="Flame" size={16} className={getStreakColor(user?.streak)} />
                    <span className={`font-mono font-semibold ${getStreakColor(user?.streak)}`}>
                      {user?.streak} days
                    </span>
                  </div>
                </td>
                  ...
                  <td className="p-4">
                    <div className="flex items-center gap-1">
                      {user?.recentAchievements?.slice(0, 3)?.map((achievement, idx) => (
                        ...
                        >
                          <Icon name={achievement?.icon} size={12} className="text-primary" />
                        </div>
                      ))}
                      {user?.recentAchievements?.length > 3 && (
