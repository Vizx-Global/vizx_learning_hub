      ...
      <div className="bg-card border border-border rounded-lg p-6">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            Complete learning modules to earn your first achievement!
          </p>
        </div>
      </div>
    );
    ...
    <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs text-muted-foreground">
              {recentAchievements?.length} new this week
            </span>
      ...
      <div className="p-4">
        ...
        {recentAchievements?.[currentIndex] && (
          <div className={`
            relative p-6 rounded-lg mb-4 overflow-hidden
            bg-gradient-to-r ${getAchievementColor(recentAchievements?.[currentIndex]?.type)}
            ${celebratingId === recentAchievements?.[currentIndex]?.id ? 'animate-pulse' : ''}
          `}>
            <div className="relative z-10 text-white">
              <div className="flex items-center gap-4 mb-3">
                <div className="w-12 h-12 bg-white/20 rounded-full flex items-center justify-center">
                  <Icon 
                    name={getAchievementIcon(recentAchievements?.[currentIndex]?.type)} 
                    size={24} 
                    className="text-white" 
                  />
                </div>
                <div className="flex-1">
                  <h4 className="font-bold text-lg">{recentAchievements?.[currentIndex]?.title}</h4>
                  <p className="text-white/80 text-sm">{recentAchievements?.[currentIndex]?.description}</p>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <Image
                    src={recentAchievements?.[currentIndex]?.userAvatar}
                    alt={recentAchievements?.[currentIndex]?.userName}
                    className="w-8 h-8 rounded-full border-2 border-white/50"
                  />
                  <div>
                    <div className="font-semibold">{recentAchievements?.[currentIndex]?.userName}</div>
                    <div className="text-white/80 text-xs">{recentAchievements?.[currentIndex]?.timeAgo}</div>
                  </div>
                </div>
                
                <button
                  onClick={() => handleCelebrate(recentAchievements?.[currentIndex]?.id)}
                  className="flex items-center gap-2 px-3 py-1 bg-white/20 rounded-full hover:bg-white/30 transition-colors"
                >
                  <Icon name="Heart" size={16} />
                  <span className="text-sm">{recentAchievements?.[currentIndex]?.celebrations || 0}</span>
                </button>
              </div>
            </div>
            ...
            {celebratingId === recentAchievements?.[currentIndex]?.id && (
              <div className="absolute inset-0 pointer-events-none">
                ...
                <div className="absolute bottom-4 left-8 animate-bounce delay-200">
            ...
            {recentAchievements?.map((achievement) => (
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  >
                    <Icon name="Heart" size={14} className="text-muted-foreground hover:text-destructive" />
                  </button>
                </div>
              </div>
            ))}
        ...
        <div className="mt-4 pt-4 border-t border-border">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { type: 'milestone', count: 12, label: 'Milestones' },
              { type: 'streak', count: 8, label: 'Streaks' },
              { type: 'completion', count: 15, label: 'Completions' },
              { type: 'competition', count: 5, label: 'Competitions' }
            ]?.map((category) => (
              <div key={category?.type} className="text-center p-3 bg-muted/20 rounded-lg">
                <div className={`
                  ...
                  bg-gradient-to-r ${getAchievementColor(category?.type)}
                `}>
                  <Icon 
                    name={getAchievementIcon(category?.type)} 
                    size={14} 
                    className="text-white" 
                  />
                </div>
                <div className="font-semibold text-foreground text-sm">{category?.count}</div>
